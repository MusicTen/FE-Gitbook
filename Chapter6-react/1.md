...
```javascript
├── package-lock.json // 锁定安装时的包的版本号，并且需要上传到git，以保证其他人在npm install时大家的依赖能保证一致,对整个文件的描述,为的是让开发者知道只要你保存了源文件，到一个新的电脑上开发，只要按照这个package-lock.json所标示的具体版本下载依赖库包，就能确保所有库包与你上次安装的完全一样，它是npm install自动生成的一文件
...
这是简单的基础项目，它的核心：react / react-dom 两个框架
特点：
   ...
   render((
       <Router> //开始创建路由表
           <Route path="/" component={ App }> // 声明每一个路由
               ...
               <Route path="users" component={ Users }> // 每个路由对应一个Component
...
- 服务端配置
  ...
  // 在你应用 JavaScript 文件中包含了一个 script 标签的 index.html 中处理任何一个 routeapp.get('*', function (request, response){response.sendFile(path.resolve(__dirname, 'public', 'index.html'))})
...
被称为下一代 Ajax 技术，采用 Promise 方式来处理数据。
...
- 提供您自己的请求对象
  您可以使用 Request() 构造函数创建请求对象，并将其作为 fetch() 方法参数传入，而不是将要请求的资源的路径传递到 fetch() 调用中。
